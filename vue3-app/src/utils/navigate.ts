...
// vue-router的push方法
export const push = (path: string, setQuery?: QueryParams, removeQuery?: string[]): void => {
  const pathInfo = createPathInfo(path, setQuery, removeQuery)
  router.push({
    path: pathInfo.path,
    query: pathInfo.search
  })
}

// vue-router的replace方法
export const replace = (path: string, setQuery?: QueryParams, removeQuery?: string[]): void => {
  const pathInfo = createPathInfo(path, setQuery, removeQuery)
  router.replace({
    path: pathInfo.path,
    query: pathInfo.search
